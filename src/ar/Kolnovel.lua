-- {"id":19691969,"ver":"3.0.0","libVer":"1.0.0","author":"ilyexZ"}
...
-- Helper function to extract chapter numbers from text
...
local function parseNovelStatus(statusText)
    if not statusText then return 3 end -- Unknown
    
    local status = statusText:lower():gsub("الحالة:%s*", ""):gsub(":%s*", ""):gsub("%s+", " ")
    
    -- Completed
    if status:find("مكتملة") or status:find("completed") or status:find("complété") or 
       status:find("completo") or status:find("completado") or status:find("tamamlandı") then
        return 1
    -- Ongoing
    elseif status:find("مستمرة") or status:find("ongoing") or status:find("en cours") or 
        ...
        return 0
    -- Hiatus
    elseif status:find("متوقفة") or status:find("hiatus") or status:find("en pause") or 
           status:find("hiato") or status:find("pausa") or status:find("pausado") or 
           status:find("duraklatıldı") then
        return 2
    else
        return 3 -- Unknown
    end
...
local function safeGETDocument(url)
    ...
    if title then
        ...
        if titleText then
            ...
            if titleText:find("bot verification") or 
               titleText:find("you are being redirected") or
               titleText:find("un instant") or
               titleText:find("just a moment") or
               titleText:find("redirecting") or
...
return {
            ...
            for _, selector in ipairs(articleSelectors) do
                ...
                if articles:size() > 0 then
                    for i = 1, articles:size() do
                        ...
                        local linkElement = article:selectFirst("a[title]") or
                                          article:selectFirst("a[href*='knovel']") or
                        ...
                        if linkElement then
                            ...
                            if title and link and title ~= "" then
                                ...
                                local imgElement = article:selectFirst("img[data-src]") or
                                                 article:selectFirst("img[src]") or
                                                 article:selectFirst("img")
                                
                                local imageURL = nil
                                if imgElement then
                                    imageURL = extractAttr(imgElement, "data-src") or 
                                             extractAttr(imgElement, "src")
                                end
            ...
            return map(d:select("article, div.bsx, div.post"), function(v)
                ...
                if linkElement and titleElement then
                    return Novel {
                        ...
                        imageURL = imgElement and (extractAttr(imgElement, "data-src") or extractAttr(imgElement, "src")) or nil,
    ...
    parseNovel = function(novelURL, loadChapters)
        ...
        local imgSelectors = {".ts-post-image", "div.bigcover img", "img[data-src]", ".novel-cover img", ".book-cover img"}
        for _, selector in ipairs(imgSelectors) do
            local imgElement = document:selectFirst(selector)
            if imgElement then
                local imageURL = extractAttr(imgElement, "data-src") or 
                               extractAttr(imgElement, "src")
                if imageURL then
        ...
        local statusElements = selectAllFromSelectors(document, {
            "div.info-content span", ".spe span", ".serl span", ".sertostat", 
            ".novel-info span", ".book-info span", "div.novel-details span"
        })
        
        for _, element in ipairs(statusElements) do
            local statusText = extractText(element)
            if statusText and (statusText:find("الحالة") or statusText:find("[Ss]tatus") or statusText:find("حالة")) then
        ...
        -- Authors extraction
        local authors = {}
        local authorElements = selectAllFromSelectors(document, {
            "div.info-content span a", ".spe a", ".serl a", 
        ...
        -- Genres extraction
        local genres = {}
        local genreElements = selectAllFromSelectors(document, {
            "div.genxed a", ".sertogenre a", ".genre-info a", 
    ...
    getPassage = function(chapterURL)
        ...
        -- Final fallback: try to get any article content
        local articleParagraphs = document:select("article p")
        if articleParagraphs:size() > 0 then
            local content = {}
            for i = 1, articleParagraphs:size() do
                local pText = extractText(articleParagraphs:get(i-1))
                if pText ~= "" then
                    table.insert(content, pText)
                end
            end
